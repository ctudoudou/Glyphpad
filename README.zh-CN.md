...
Glyphpad 是我想在 macOS 上继续用到的那个 Launchpad：打开，输入几个字母，启动应用，然后它安静退场。

它刻意做得熟悉：全屏应用网格、搜索、文件夹、分页、拖拽排序、快捷键、自定义背景，还有一个独立设置窗口。不是网页壳，也不是臃肿的控制中心。当前本地 app bundle 仅约 **3 MB**。

重要数据都留在本机 SQLite：应用列表、布局、文件夹、设置，以及后续分类历史。大模型辅助整理会是帮手，不会替你接管本机数据。
...
- **仅约 3 MB**：像一个真正的小工具，而不是又一个平台。
- **就是 Launchpad 的感觉**：全屏网格、柔和虚化背景、搜索、文件夹、分页和分页圆点。
- **打开就能用**：搜索框默认聚焦，想找什么直接打字。
- **布局听你的**：拖拽排序、创建文件夹、拖入拖出，顺序都会保存。
- **空文件夹自动清理**：文件夹空了就消失，不留垃圾。
- **两种浏览方式**：应用多就纵向滚动，想要经典节奏就横向分页。
- **外观可以自己调**：背景图片、虚化强度、行数、列数、图标大小和自动排列都能改。
- **设置不打扰启动器**：启动器保持干净，布局、外观、快捷键和 API 配置放在独立窗口。
- **快捷键友好**：用默认全局快捷键，也可以录制自己的快捷键。
- **本地优先**：应用元数据、文件夹、布局、设置和后续分类数据都存在本机 SQLite。
